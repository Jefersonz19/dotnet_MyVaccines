...
namespace MyVaccine.WebaApi.Services.Implementations;
    ...
    public async Task<AuthResponseDto> RefreshToken(string email)
    {
        ...
        try
        {
            ...
            if (user != null)
            {
                ...
                var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(Environment.GetEnvironmentVariable(MyVaccineLiterals.CONNECTION_STRING)));
                var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
                var token = new JwtSecurityToken(
                    claims: claims,
                    expires: DateTime.Now.AddMinutes(15),
